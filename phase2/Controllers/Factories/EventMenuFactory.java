...
public class EventMenuFactory {
     ...
     */
    public EventMenuController getEventMenu() {
        if (userManager.getCurrentUser().getClass().equals(Attendee.class)) {
            ...
            return new AttendeeEventController(attendeeManager, eventManager, roomManager, languageManager, frame);
            ...
            return new OrganizerEventController(organizerManager, roomManager, eventManager, userManager, languageManager, speakerManager, accountCreatorFactory, frame);
        } else if(userManager.getCurrentUser().getClass().equals(Speaker.class)){
            SpeakerManager speakerManager = factoryUseCaseHelper.createSpeakerManager();
            speakerManager.setCurrentUser(userManager.getCurrentUser());
            return new SpeakerEventController(speakerManager, eventManager, roomManager, languageManager, frame);
        } else {
            AdminManager adminManager = factoryUseCaseHelper.createAdminManager();
            adminManager.setCurrentUser(userManager.getCurrentUser());
            return new AdminEventController(userManager, eventManager, roomManager, languageManager, factoryUseCaseHelper.createSpeakerManager(), adminManager, frame);
        }
    }
}
